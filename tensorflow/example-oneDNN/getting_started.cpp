...
void getting_started_tutorial(engine::kind engine_kind) {
    ...
    cl::sycl::buffer<float,1> src_buffer(image.data(),image.size());
    // auto src_mem = memory(src_md, eng);
    auto src_mem = dnnl::sycl_interop::make_memory(src_md, eng, sycl_interop::memory_kind::buffer);
    write_to_dnnl_memory(image.data(), src_mem);

    cl::sycl::buffer<float,1> dst_buffer(image.data(),image.size());
    // For dst_mem the library allocates buffer
    // auto dst_mem = memory(src_md,eng);
    auto dst_mem = dnnl::sycl_interop::make_memory(src_md, eng, sycl_interop::memory_kind::buffer);
    // [Create memory objects]
